...
def geometry(z):
    ...
    beta = sp.atan((h_Fs-h_Bs)/l_Top) #angle of bottom flange
...
def centroid(area_Fs, area_Bs, area_Top, area_Bottom, area_Total, h_Fs, h_Bs, l_Top, l_Bottom, beta):
    X_Centroid = (area_Fs * l_Top + area_Top * 0.5 * l_Top + area_Bottom * 0.5 * l_Bottom *sp.cos(beta))/area_Total #X-position of the centroid
    Y_Centroid = (area_Fs * 0.5 * h_Fs + area_Bs * 0.5 * h_Bs + area_Bottom * (0.5 * l_Bottom * sp.sin(beta) + h_Bs))/area_Total #Y-position of the centroid
...
def local_Centroids(h_Fs, h_Bs, l_Top, l_Bottom, beta, X_Centroid, Y_Centroid):
    ...
    X_Centroid_Bottom = 0.5 * l_Bottom * sp.cos(beta) - X_Centroid #X-position of the centroid of the bottom plate
    Y_Centroid_Bottom = h_Bs + 0.5 * l_Bottom *sp.sin(beta) - Y_Centroid #Y-position of the centroid of the bottom plate
...
def plate_Inertia_XX(h_Fs, h_Bs, l_Top, l_Bottom, beta, Y_Centroid_Fs, Y_Centroid_Top, Y_Centroid_Bs, Y_Centroid_Bottom):
    ...
    I_XX_Bottom = (t_Bottom * l_Bottom ** 3 * (sp.sin(beta)) ** 2)/12 + t_Bottom * l_Bottom * Y_Centroid_Bottom ** 2
...
def plate_Inertia_YY(h_Fs, h_Bs, l_Top, l_Bottom, beta, X_Centroid_Fs, X_Centroid_Top, X_Centroid_Bs, X_Centroid_Bottom):
    ...
    I_YY_Bottom = (t_Bottom * l_Bottom ** 3 * (sp.cos(beta)) ** 2)/12 + t_Bottom * l_Bottom * X_Centroid_Bottom ** 2
